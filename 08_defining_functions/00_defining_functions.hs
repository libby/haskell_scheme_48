...
{--
    ...
    > compile:  ghc -o srcfiles/repl_fun 08_defining_functions/00_defining_functions.hs
...
primativeBindings :: IO Env
primativeBindings = nullEnv >>= (flip bindVars $ map makePrimativeFunc primatives) -- TODO: This is like casting
            where makePrimativeFunc (var, fn) = (var, PrimitiveFunc fn)
...
runRepl :: IO ()
runRepl = primativeBindings >>= until_ (== "quit") (readPrompt "Lisp>>> ") . evalAndPrint 

runOne :: String -> IO()
runOne input = primativeBindings >>= flip evalAndPrint input
